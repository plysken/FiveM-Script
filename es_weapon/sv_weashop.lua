...
AddEventHandler('CheckMoneyForWea', function(weapon,price)
	TriggerEvent('es:getPlayerFromId', source, function(user)
		...
		if (tonumber(user.money) >= tonumber(price)) then
			...
			local result = MySQL:getResults(executed_query, {'weapons'}, "identifier")
			if result then
			...
			if (tonumber(max_number_weapons) > tonumber(nb_weapon)) then
				...
				MySQL:executeQuery("INSERT INTO users (identifier,weapons,withdraw_cost) VALUES ('@username','@weapon','@cost')",
...
AddEventHandler("weaponshop:GiveWeaponsToPlayer", function(player)
	TriggerEvent('es:getPlayerFromId', player, function(user)
		...
		local executed_query = MySQL:executeQuery("SELECT * FROM users WHERE identifier = '@username'",{['@username'] = playerID})
		local result = MySQL:getResults(executed_query, {'weapons','withdraw_cost'}, "identifier")
	
		delay = 2000
		if(result)then
			for k,v in ipairs(result) do
				if (tonumber(user.money) >= tonumber(v.withdraw_cost)) then
					TriggerClientEvent("giveWeapon", player, v.weapons, delay)
